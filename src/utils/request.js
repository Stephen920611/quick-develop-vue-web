...
service.interceptors.request.use(
  config => {
    if (getToken()) {
      ...
      // setToken('harbour_process_monitor_token',{"token":"Bearer eyJhbGciOiJIUzUxMiJ9.eyJpc3MiOiJjeWxpYW4iLCJzdWIiOiI3MTI1IiwiaWF0IjoxNjA2Mzk4NjgwfQ.nzLET8aq2MfMe3NQHeoqE6zRIRhIahouUmhZFrrFUZa4u96Et3oalVsEed7s5VLiTxjz_NCgigcSQnETfaeoLA"});
      // config.headers['Content-Type'] = 'application/x-www-form-urlencoded'
    }
...
service.interceptors.response.use(
  ...
  response => {
    ...
    if (res.code !== 2000 && res.code !== 4002 && res.code !== 4003) {
      Message({
        message: res.message || 'Error',
        type: 'error',
        duration: 5 * 1000
