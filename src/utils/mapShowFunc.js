...
_this.mapVariable = {
    ...
    options: {
        ...
        mapTypeControlOptions: {
            ...
            position: 3,  //地图类型插件放于地图的右上角:1--左上角  2--正上方   3--右上角
            // position : google.maps.ControlPosition.TOP_RIGHT,//地图类型插件放于地图的右下角
        }
    },
    _eventHandler: '',     //绘制句柄
...
_this.addMarker = (map, id, lat, lng, icon, title, type, flag, showFlag, callback) => {//添加点位
    ...
    if ((type == "temp" || type == "car") && callback) {
    ...
    google.maps.event.addListener(marker, 'click', (event) => {
        ...
        if (type == "camera" || type == 'car' || type == '' || type == 'tank' || type == 'device') {
            _this.closeInfoWin();
            _this.showMarkerInfo(map, id, marker, type);   //显示点位信息
        }
        if (type == 'company'||type == 'blast') {
            //若是地图级别小于15级，则将地图放大至15级；若是大于或等于15级，则保持当前级别
...
_this.showMarkerInfo = (map, id, marker, type) => {
    ...
    switch(type){
        case 'camera':
            const statusName = info[5]==1?'在线':'离线';
            const statusColor = info[5]==1?'green':'red';
            content = '<div class="loc-content loc-content-1">'
                + '<div class="msg-content">'
                + '<p class="msg-info title"><span>' + info[3] + '</span></p>'
                + '<p class="msg-info"><span>企业：' + info[6] + '</span></p>'
                + '<p class="msg-info"><span >状态：</span><span style="color: '+statusColor+'">'+statusName+'</span></p>'
                + '<p class="msg-info"><span class="location"><i class="square"></i><span class="loc loc-lng">E ' + parseFloat(info[2]).toFixed(5) + '</span></span>'
                + '<span class="location"><i class="square"></i><span class="loc loc-lat">N ' + parseFloat(info[1]).toFixed(5) + '</span></span></p>'
                // +'<p class="msg-info"><span>所属区域：'+info[4]+'</span></p>'
                + '</div>';
            content += '<div class="icon-wrap">'
                + '<span class="icon" onclick="mapShowFunc.openCameraVideoDialog(\''+info[0]+'\',\''+info[3]+'\')">实时预览</span>'
                + '</div>';
            content += '</div>';
            break;
        case 'car':
        ...
        case 'company':
            content = '<div class="loc-content loc-content-1">'
                ...
                + '<div class="msg-content">'
                + '<p class="msg-info title ellipsis-l1" title="'+info[0]+'"><span>'+info[0]+'</span></p>'
                + '<p class="msg-info ellipsis-l1" title="'+info[1]+'"><span>企业地址：'+info[1]+'</span></p>'
                + '<p class="msg-info ellipsis-l1" title="'+info[3]+'"><span>企业类型：'+info[3]+'</span></p>'
                + '<p class="msg-info"><span>责任人：'+info[4]+'</span></p>'
                + '<p class="msg-info"><span>责任人电话：'+info[5]+'</span></p>'
                + '</div>'
                + '<div class="icon-wrap">'
                ...
                + '<span class="icon" onclick="mapShowFunc.openDangerSource()">危险源</span>'
                + '</div>';
            +'</div>';
            break;
        case 'dangerSource':
            content = '<div class="loc-content loc-content-3">'
                // + '<div class="loca-info">信息展示</div>'
                + '<div class="msg-content">'
                + '<p class="msg-info title"><span>' + info[0] + '</span></p>'
                + '<p class="msg-info">所在地址：<span>' + info[1] + '</span></p>'
                + '</div>'
                /*+ '<div class="icon-wrap">'
                + '<span class="icon" onclick="mapShowFunc.openVideoMonitorDialog()">实时视频</span>|'
                + '<span class="icon" onclick="mapShowFunc.openEmergencyDialog()">应急预案</span>'
                + '</div>';*/
                +'</div>';
            break;
        case 'bayonet':
            content = '<div class="loc-content loc-content-3">'
                ...
                + '<p class="msg-info title"><span>' + info[0] + '</span></p>'
                + '<p class="msg-info">所在地址：<span>' + info[1] + '</span></p>'
                + '<p class="msg-info">车牌号：<span>' + info[2] + '</span></p>'
                + '<p class="msg-info">驾驶员：<span>' + info[3] + '</span></p>'
                + '<p class="msg-info">出入状态：<span>'+ (info[4]==1?'进厂':'出厂') +'</span></p>'
                + '<p class="msg-info">核验状态：<span>'+ (info[5]==1?'核验正常':'核验异常') +'</span></p>'
                + '<p class="msg-info">核验时间：<span>'+ (info[4]==1?info[6]:info[7]) +'</span></p>'
                + '</div>'
                +'</div>';
            break;
        case 'tank':
            const icon = marker.getIcon();
            if(icon.indexOf('storehouse32')>-1){
                //储罐的原始图标弹出的气泡内容
                content = '<div class="loc-content loc-content-1">'
                    // + '<div class="loca-info">信息展示</div>'
                    + '<div class="msg-content">'
                    + '<p class="msg-info title"><span>'+info[1]+'</span></p>'
                    + '<p class="msg-info ellipsis-l1" title="'+info[2]+'">所属企业：<span>'+info[2]+'</span></p>'
                    + '<p class="msg-info">负责人：<span>'+info[3]+'</span></p>'
                    + '<p class="msg-info">负责人电话：<span>'+info[4]+'</span></p>'
                    + '</div>'
                    + '<div class="icon-wrap">'
                    + '<span class="icon" onclick="mapShowFunc.openTankDetailDialog(\''+info[0]+'\')">详情</span>'
                    + '</div>';
                +'</div>';
            }else{
                //储罐的报警图标弹出的气泡内容
                const liquidColor = info[13] == 1?'#FF1610':'#333';
                const tempColor = info[14] == 1?'#FF1610':'#333';
                const pressureColor = info[15] == 1?'#FF1610':'#333';

                content = '<div class="loc-content loc-content-4">'
                    // + '<div class="loca-info">信息展示</div>'
                    + '<div class="msg-content">'
                    + '<p class="msg-info title"><span>'+info[1]+'</span></p>'
                    + '<p class="msg-info ellipsis-l1" title="'+info[2]+'">所属企业：<span>'+info[2]+'</span></p>'
                    + '<p class="msg-info ellipsis-l1" title="'+info[5]+'">报警内容：<span>'+info[5]+'</span></p>'
                    + '<p class="msg-info ellipsis-l1" title="'+info[6]+'">报警原因：<span>'+info[6]+'</span></p>'
                    + '<p class="msg-info">报警时间：<span>'+info[7]+'</span></p>'
                    + '<p class="msg-info ellipsis-l1" title="'+info[8]+'">处置措施：<span>'+info[8]+'</span></p>'
                    + '<p class="msg-info ellipsis-l1" title="'+info[9]+'">处置结果：<span>'+info[9]+'</span></p>'
                    + '<p class="msg-info ellipsis-l1" title="'+info[3]+'">负责人：<span>'+info[3]+'('+info[4]+')</span></p>'
                    // + '<p class="msg-info ellipsis-l1" title="'+info[4]+'">联系电话：<span>'+info[4]+'</span></p>'
                    + '<p class="msg-info">'
                    + '<span class="margin-r-10">液位：<span class="font-size-20 font-bold" style="color:'+liquidColor+'">'+info[10]+'</span>m</span>'
                    + '<span class="margin-r-10">温度：<span class="font-size-20 font-bold" style="color:'+tempColor+'">'+info[11]+'</span>℃</span>'
                    + '<span class="margin-r-10">压力：<span class="font-size-20 font-bold" style="color:'+pressureColor+'">'+info[12]+'</span>MPa</span></p>'
                    + '</div>'
                    +'</div>';
            }
            break;
        ...
        case 'device':
            console.log('device');
            content = '<div class="loc-content loc-content-5">'
                + '<div class="msg-content">'
                + '<p class="msg-info title ellipsis-l1"><span class="font-size-16">' +info[1] +'</span></p>'
                + '</div>'
                + '<div class="icon-wrap">'
                + '<span class="icon" onclick="mapShowFunc.openDeviceDetailFlag(\''+info[0]+'\')" >查看更多</span>'
                + '</div>';
            + '</div>'
            break;
        case 'blast':
            content = '<div class="loc-content loc-content-1">'
                // + '<div class="loca-info">信息展示</div>'
                + '<div class="msg-content">'
                + '<p class="msg-info title ellipsis-l1" title="'+info[1]+'"><span>'+info[1]+'</span></p>'
                // + '<p class="msg-info ellipsis-l1" title="'+info[5]+'"><span>起止时间：'+info[5]+'</span></p>'
                + '<p class="msg-info ellipsis-l1" title="'+info[2]+'"><span>地理位置：'+info[2]+'</span></p>'
                + '<p class="msg-info ellipsis-l1" title="'+info[4]+'"><span>类型：'+info[4]+'</span></p>'
                // + '<p class="msg-info"><span>责任人：'+info[4]+'</span></p>'
                // + '<p class="msg-info"><span>责任人电话：'+info[5]+'</span></p>'
                + '</div>'
                + '<div class="icon-wrap">'
                + '<span class="icon" onclick="mapShowFunc.openBlastDetailFlag(\''+info[0]+'\')" >查看更多</span>'
                + '</div>';
            +'</div>';
            break;
    }
    ...
    /*if (type == "camera") {
        ...
        const icon = marker.getIcon();
        if(icon.indexOf('storageTank16')>-1){
            //储罐的原始图标弹出的气泡内容
            content = '<div class="loc-content loc-content-1">'
                // + '<div class="loca-info">信息展示</div>'
                + '<div class="msg-content">'
        ...
        content = '<div class="loc-content loc-content-2">'
            + '<div class="msg-content">'
            + '<p class="msg-info"><span class="font-size-16">' +info[0] +'</span></p>'
            + '</div>'
            + '</div>';
    }*/
 ...
 * 打开摄像机的实时视频
 */
_this.openCameraVideoDialog = (camCode,camName)=>{
    //调用组件中的方法，打开摄像机实时视频窗口
    vm.openCameraVideoDialog(camCode,camName);
};
...
/*打开应急预案的窗口*/
_this.openEmergencyDialog = ()=>{
    vm.egDialogVisible = true;
};

/*打开实时视频监控的窗口*/
_this.openVideoMonitorDialog = ()=>{
    vm.videoMonitorDialogVisible = true;
};

/*打开储罐的详情窗口*/
_this.openTankDetailDialog = (id)=>{
    const data = vm.regionTanksHash.getValue(id)
    // console.log(vm.regionTanksHash,'regionTanksHash');
    vm.openDialog('seeTankDetailFlag',data);
};

/*打开爆破工程的详情窗口*/
_this.openBlastDetailFlag = (id)=>{
    const data = vm.regionBlastHash.getValue(id)
    console.log(id,data,vm.regionBlastHash,'openBlastDetailFlag');
    vm.openDialog('seeBlastDetailFlag',data);
};

/*打开矿山的详情窗口*/
_this.openDeviceDetailFlag = (id)=>{
    const data = vm.regionBridgesHash.getValue(id);
    //暂时传id
    // vm.openDialog('seeDeviceDetailFlag',id);
    // vm.$router.push({path: "/map-monitor/mine-detail", query: {id: id}})
    let routeData = vm.$router.resolve({path: "/map-monitor/mine-detail", query: {id: id}});
