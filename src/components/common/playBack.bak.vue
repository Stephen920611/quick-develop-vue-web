  ...
  <div id="playBackContainer" ref="playBackContainer">
    <div class="multiple-win-wrap" :class="{'full-screen':$store.state.video.isFullScreen}">
      <video-comm
        ...
        :row-num-style="curRowCls.join('')"
        :win-num="winCount"
        :cur-win-num="curWinCount"
        :win-select-data="winSelectData"
        @update-data="updateData"
        @toggle-win="handleToggleWin"
      />
    </div>
    <div v-show="(!$store.state.video.isFullScreen && isShowBtn)" id="multiple-channel-choose">
      <df-drop-down
        name="窗口选择"
        type="primary"
        :data="winSelectData"
        :default-value="curVideoRowCls"
        @command="handleChooseWin"
      />
    ...
    <div id="timeLineBox" ref="timeLineBox">
      <template v-for="(item,index) in timeBarsComp">
        <TimeLine
          ...
          :width="timeParam.canvasWidth"
          :height="62"
          @click-bar="handleClickBar"
          @click-stop-video="stopVideoPlay(index)"
        />
      </template>
    </div>
  </div>
...
export default {
      ...
      instance: function($vue, videoId, cameraid, sessionid, authcode, browserInfo) {
        ...
        this.ReconnectFunction = function() {
          ...
          if (self.bNeedReconnect === true) {
            ...
            const winIndex = parseInt(self._videoId.substring(5, self._videoId.length)) - 1
        ...
        this.H5MWebSocketClient = function() {
          ...
          try {
            if (window.location.protocol == 'http:') {
              ...
              const h5ServerIp = localStorage.getItem('h5_manager_ip')
        ...
        this.ReadFromBuffer = function() {
          ...
          try {
            ...
            if (self._type == 'Playback') {
              ...
              const temp = new Date(self._start_time).getTime() + self.video.currentTime * 1000
              const dateFormat = dateTime.dateFormat(new Date(temp))
        ...
        this.MediaSourceOpen = function() {
          ...
          // 得到视频窗口
          const winNumber = self._videoId.substring(5, self._videoId.length)
          const winNum = 'win' + winNumber
          try {
            // 开启视频加载动画
        ...
        this.doCmd = function(cameraid, cmd, speed, param1, param2) {
          if (self.managerSocket != null) {
            ...
            const jsondata = JSON.stringify(data) // 将JSON对象序列化为JSON字符串
        ...
        this.RecvManager = function(evt) {
          ...
          const winIndx = parseInt(self._videoId.substring(5, self._videoId.length)) - 1
          if (jsondata.Header.Status != '200') { // 设置错误提示码
  ...
  methods: {
    ...
    handleFullScreen() {
      ...
      const domObj = document.querySelector('#playBackContainer')
    ...
    getCurrProgressBar() {
      // 获取当前时间轴组件的时间轴对象
      const winInx = this.$refs.videoComm.checkedVideoWin
    ...
    chooseDateAndPlayBack(startDate, startT, endT) {
      // 获取时间轴对象
      const currInstance = this.getCurrProgressBar()
    ...
    playSingleVideo(camObj, startTime, endTime, type) {
      ...
      // 2.查找被选中的窗口是否正在播放
      const videoWinSelectArr = self.$refs.videoComm.videoWinSelect
    ...
    videoPlay(videoWinSelect, type, camName, cameraType, cameraId, startTime, endTime) {
      const self = this
      const videoId = videoWinSelect.videoId
      const winNum = videoWinSelect.winId
      const playState = videoWinSelect.playState
      ...
      // 将窗口对象添加至_winObjHash
      const winValue = { 'cameraId': cameraId }
    ...
    stop(winNum) {
      const self = this
      const instance = self._videoObjHash.getValue(winNum)
      if (instance != null) {
        if (instance._type == 'Play') {
          instance.StopRealPlay()
        } else {
    ...
    initInstance(videoId, cameraid, sessionid, authcode, winNum, browserInfo, cameraType, camName) {
      const self = this
      const instance = new self.instance(this, videoId, cameraid, sessionid, authcode, browserInfo)
      if (cameraType) {
    ...
    getWinNumByCameraId(value) {
      const entry = this._winObjHash.getobject()
    ...
    videoPlaySubFunc(winNum, type) {
      const self = this
      let videoObj
      const winsArr = self._videoObjHash.getKeys()
      if (winNum) { // 播放指定视频
      ...
      function videoPlayInWin(num) {
        const instance = self._videoObjHash.getValue(num)
        if (type == 'playBack') {
          // 启动暂停的时间轴
          self.$refs.timeline[self.checkedWinNum].setTimeMoveFunc(true)
    ...
    closeAppointVideo(winNum, type) {
      ...
      if (winNum) {
        ...
        self.videoPause(true, winNum, type)
        // 清空视频对象状态hash
        const index = winNum.substring(3) - 1
    ...
    videoPause(close, winNum, type, notCheckFirst) {
      ...
      function videoPause(num) {
        ...
        const winIndex = parseInt(num.substring(3, num.length)) - 1
        ...
        function clearObj(key, winIndex) {
          ...
          // 删除timeBar对应窗口的录像块
          const timeBar = self.timeBarsComp[self.checkedWinNum].timeBars[winIndex]
    ...
    videoPauseSubFunc(close, winNum, winObj, type) { // 窗口号、窗口对象
      const videoObj = this._videoObjHash.getValue(winNum)
    ...
    closeAllVideo() {
      ...
      // 选中窗口定位到第一个
      this.$refs.videoComm.checkedVideoWin = 0
      const videoWinSelectArr = this.$refs.videoComm.videoWinSelect
      videoWinSelectArr.forEach((item, index) => {
        if (index == 0) {
          item.checked = true
        } else {
    ...
    handleCtrl(winSelect, cmd, dir) {
      var self = this
      const instance = self._videoObjHash.getValue(winSelect)
      const speed = this.ctrlDefaultValue ? this.ctrlDefaultValue : 90
      console.log('云台控制速度：', speed)
      if (instance) {
        const cameraCode = instance._cameraid
