...
//            this.returnTime = this.startTime;
        ...
        methods: {
            ...
            updateTimeCell(value) {
                this.clearCanvas();
                this.timecell = value;
                this.initCanvas(this.start_timestamp, value, this.end_timestamp, this.returnTime)
            ...
            updateReturnTime(value) {
                ...
                this.initCanvas(this.start_timestamp, this.timecell, this.end_timestamp, value)
            },
             ...
             * 根据传入的时间（秒），刻度值移动
             */
            setBarTimeMove(moveTime) {
                //默认移动到当前位置
                this.returnTime = this.returnTime + moveTime * 1000;
                console.log(this.returnTime,"this.returnTime");
                this.move_to_return_time(this.start_timestamp, this.returnTime, this.end_timestamp, this.timecell, true);
            },

            /**
             * 刻度线稍微移动一点，为了让切换窗口是让时间轴的刻度显示出来
             */
            setTimeMoveFunc: function (flag) {
                let self = this;
                if (flag) {
                    self.setTimeMove = setInterval(function () {
                        ...
                        self.setTimeMove && clearInterval(self.setTimeMove);   //清除定时器后开启
             ...
             */
            /*setTimeMoveFunc: function (flag, isClose = true,time = 1000) {
...
//                    this.timeLineContainer.addEventListener('dblclick', this.dblclickFunc.bind(this));
            ...
            clickFunc: function (e) {
                ...
                if (this.end_timestamp > temporaryTime) {
                    ...
                    this.$message({message: "当前时间无录像", type: 'warning', showClose: true})
                }
            },
            /**
             * 双击事件
