  ...
  <div class="multiple-video-show">
    ...
    <ul id="video-content" :class="rowNumStyle">
      <li
        ...
        :key="index"
        class="win"
        :class="{'checked':index==checkedVideoWin}"
      ...
      >
        ...
        >
          ...
          <div v-for="(dir,d) in dirSelectData" v-show="showInfo.dirShow[index]" :key="d" class="dir-group">
            <i class="dir" :class="dir.id" :data-type="dir.type" :title="dir.name" @mousedown="handleDirMouseDown(index,dir.type,dir.name)" @mouseup="handleDirMouseUp(index,dir.type,dir.name)" />
          </div>
          <div>
            ...
            <div v-show="showInfo.operateShow[index]" class="video-operate">
              <ul class="video-operate-ul">
                ...
                <li v-show="playState=='realPlay'?false:true" class="float-right video-play" title="播放" @click.stop="handleOperBtn(0,index)">
                  <i class="fa fa-play" aria-hidden="true" />
                </li>
                <li v-show="playState=='realPlay'?false:true" class="float-right video-pause" title="暂停" @click.stop="handleOperBtn(1,index)">
                  <i class="fa fa-pause" aria-hidden="true" />
                </li>
                <li class="float-right video-close" title="关闭" @click.stop="handleOperBtn(2,index)">
                  <i class="fa fa-times" aria-hidden="true" />
                </li>
                <li v-show="false" class="float-right video-cut" title="截图" @click.stop="handleOperBtn(3,index)">
                  <a class="capture" href="javascript:;">
                    <i class="fa fa-camera":style="showInfo.operBtnActiveColor[index][3]" aria-hidden="true" />
                  </a>
                </li>
                <li class="float-right video-illegal" title="手动抓拍" @click.stop="handleOperBtn(4,index)">
                  <a class="capture" href="javascript:;">
                    <i class="fa fa-camera-retro" :style="showInfo.operBtnActiveColor[index][4]" aria-hidden="true" />
                  </a>
                </li>
                <li class="float-right video-ctrl" title="云台控制" @click.stop="handleOperBtn(5,index)">
                  <a class="capture" href="javascript:;">
                    <i class="fa fa-arrows" :style="showInfo.operBtnActiveColor[index][5]" aria-hidden="true" />
                  </a>
                </li>
                <li v-show="false" class="float-right video-locate" title="3D定位"@click.stop="handleOperBtn(6,index)">
                  <a class="capture" href="javascript:;">
                    <i class="fa fa-location-arrow" :style="showInfo.operBtnActiveColor[index][6]" aria-hidden="true" />
                  </a>
                </li>
              </ul>
            </div>
...
import { checkUserCameraPower, uploadImageBase64 } from '@/api/video'
...
export default {
  ...
  watch: {
    'winNum': {
      handler(newValue) {
        // 根据窗口的数量调整暂停或启动按钮的大小
        const fontSize = this.setWinPlayBtnSize(newValue)
          ...
          if (curWinPlayState == 'Play') {
            ...
            checkUserCameraPower(params).then(res => {
              if (res.data) {
                ...
                uploadImageBase64({ data: imgBase64 }).then(res => {
                  if (res.code == '1000') {
                    // Base64图片上传
                    const param = {
