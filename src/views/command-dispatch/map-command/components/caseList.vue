<template>
    <div class="case-container">
        <div class="search-form">
            <el-form ref="form" :model="form" size="mini">
                ...
                <el-row :gutter="10">
                    <el-col :span="12">
                        <el-form-item label="">
                            <el-input v-model="form.address"  placeholder="请输入案件地址"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
        ...
        <div class="list" v-scrollBar v-loading="loading">
            <div class="cell" v-for="(item,index) in caseList" :key="index" @click="handleClick(item)" :class="{'active':multiSelect.indexOf(item.id)>-1}">
                <el-row class="padding-td-5"><span class="title">案件编号：{{item.code}}</span></el-row>
                <el-row class="padding-td-5 ellipsis-l1"><span class="sub-title" :title="item.name">{{item.name}}</span>
                </el-row>
                <el-row class="padding-td-5 ellipsis-l1"><span class="sub-type" :title="item.address">{{item.address}}</span></el-row>
                <el-row class="padding-td-5">
                    <span class="alarm-time-title">报警时间</span>
                    <span class="alarm-time-content">{{item.caseTime}}</span>
                </el-row>
...
<script>
    ...
    import {queryCaseList} from '@/api/map/map'
    export default {
        name: "caseList",
        ...
        watch:{
            'data':{
                handler(value){
                    if(value){
                        this.caseList = value.concat(this.caseList)
                    }
                },
                deep:true,
                immediate:true
...
//            this.form.startTime = extDateTime.curentDate() + ' 00:00:00';
        ...
        methods: {
            getList(){
                ...
                this.loading = true;
                queryCaseList(this.$qs.stringify(params)).then(res=>{
                    this.loading = false;
