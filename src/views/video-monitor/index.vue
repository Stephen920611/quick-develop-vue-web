...
export default {
  ...
  provide() {
    return {
      ...
      'judgeAuthorityFunc': this.judgeAuthorityFunc,
      'uploadBase64Func': this.uploadBase64Func
        ...
        pickerTimeEnd: { // 限制结束时间
          disabledDate: (time) => {
            if (this.timeForm.startTime != '' && this.timeForm.startTime) {
              ...
              return time.getTime() < timeStr || time.getTime() > Date.now() - 8.64e6
            } else {
              return time.getTime() > Date.now() - 8.64e6
            }
          }
  ...
  methods: {
        ...
        if (res.data) {
          ...
          if (node.type.indexOf('camera') > -1) {
            ...
            if (!this.isShowTimeForm) {
              // 实时视频
              this.$refs.realPlay.playVideoByCameraId(node, '#realPlayContainer > #video-content>li', 'Play')
            } else {
              // 录像回放
            }
          }
    ...
    queryTimeForm() {
      ...
      checkUserCameraPower(params).then(res => {
        if (res.data) {
          ...
          this.$message({ message: '该用户没有对摄像机进行录像查询的权限', type: 'warning', showClose: true })
        }
      })
    },
    // 录像回放-重置查询时间
    ...
    // 1视频预览 2抓拍 3视频录像 4云台控制 5录像查询 6录像下载
    judgeAuthorityFunc(type, cameraCode, callback) {
      const params = {
        index: type,
        roleId: this.userInfo.roleId,
        userId: this.userInfo.id,
      ...
      checkUserCameraPower(params).then(res => {
        if (res.data && callback) {
          ...
          this.$message({ message: '该用户没有操作权限！', type: 'warning', showClose: true })
        }
      })
    },
    uploadBase64Func(base64, callback) {
      uploadImageBase64({ data: base64 }).then(res => {
        if (res.code == '1000' && callback) {
          callback(res.data)
        }
      })
    }
