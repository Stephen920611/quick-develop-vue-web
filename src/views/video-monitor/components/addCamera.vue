<template>
  <div id="mask" v-scrollBar class="mask-add-edit-manage" style="position: relative;max-height: calc(100vh - 300px)">
    <div id="mask-content" class="mask-content">
      <div class="info-wrap">
        ...
        <div class="el-transfer__buttons">
          <el-button class="el-button el-button--primary el-transfer__button" icon="el-icon-arrow-right" :disabled="treeBtn" @click="removeTree">右 移</el-button>
          <el-button class="el-button el-button--primary el-transfer__button" icon="el-icon-arrow-left" :disabled="checkedBtn" @click="removeCameraCode">左 移</el-button>
        </div>
        <div class="el-transfer-panel">
          <p class="el-transfer-panel__header" style="margin-top: -5px;">
          ...
          <div class="el-transfer-panel__body">
            <el-checkbox-group v-model="checkedItems">
              <el-checkbox v-for="camera in groupChecked" :key="camera.code" :label="camera.code">{{ camera.text }}</el-checkbox>
            </el-checkbox-group>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    removeTree() {
      ...
      this.checkedItems = []
      // 获取所有选中的树节点
      const checkNodes = this.$refs.initTree.$refs.tree.getCheckedNodes()
      let obj = {}
      checkNodes.map(item => {
        // 筛选出摄像机节点
        if (item.type == 'camera') {
    ...
    submitForm() {
      ...
      const obj = { rootId: this.rootData.id, camCodeList: this.camCodeList.join(','), id: this.currentNode.id, roleId: this.userInfo.roleId }
      addCam(obj).then(res => {
        this.$message({
          type: 'success',
          message: res.msg
