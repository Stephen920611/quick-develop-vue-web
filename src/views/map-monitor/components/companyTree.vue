...
<template>
  <div class="tab-container">
    ...
    <div class="tree-wrapper">
      ...
      >
        <span slot-scope="{node,data}" class="custom-tree-node ellipsis-l1" :title="node.label">
          <span class="ellipsis-l1">
            <i class="self-icon" :class="data.icon?data.icon:''" />{{ node.label }}
          </span>
        </span>
      </el-tree>
    </div>
...
export default {
  inject: ['handleSwitch', 'updateDefaultSelectSwitch', 'defaultSelectSwitch', 'locMarkerOpenInfoWindow'],
  components: {}, // 注入参数
  data() {
    return {
      form: {
            ...
            getCompanyDetail({ id: node.id }).then(res => {
              ...
              if (boundsAry && boundsAry.length > 0) {
                ...
                boundsAry.map((bounds, index) => {
                  if (bounds.length >= 2) {
                    const LatLngBounds = new google.maps.LatLngBounds()// API地图边界
                    bounds.map(ktem => {
